...
func main() {
	...
	restful.Add(NewUserServices())
	log.Print("start listening on localhost:8080")
	log.Fatal(http.ListenAndServe(":8082", nil))
}

func NewUserServices() *restful.WebService {
	...
	ws.Route(ws.GET("/{user-id}").Filter(routeLogging).Filter(NewCountFilter().routeCounter).To(findUsers))
	return ws
}
...
func getAllUsers(request *restful.Request, response *restful.Response) {
	...
	response.WriteEntity(UserLists{[]Users{{"42", "Gandalf"}, {"3.14", "Pi"}}})
}

// GET http://localhost:8080/users/42
func findUsers(request *restful.Request, response *restful.Response) {
	log.Print("findUser")
	response.WriteEntity(Users{"42", "Gandalf"})
}
